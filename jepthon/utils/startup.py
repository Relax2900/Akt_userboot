...
async def startupmessage():
    ...
    try:
        if BOTLOG:
            Config.CATUBLOGO = await jepiq.tgbot.send_file(
                ...
                caption="**᯽︙ بــوت أكثون يـعـمـل بـنـجـاح ✓ **\n**᯽︙ ارسل `.الاوامر` لرؤية اوامر السورس**",
                buttons=[(Button.url("سورس أكثون", "https://t.me/AkTSupport"),)],
            )
    except Exception as e:
        LOGS.error(e)
        return None
    ...
    try:
        if msg_details:
            ...
            text = (
                message.text
                + "\n\n**᯽︙اهلا وسهلا لقد قمت باعاده تشغيل بـوت أكثون تمت بنجاح**"
            )
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        ...
        photobt = await jepiq.upload_file(file="JepIQ/razan/resources/start/Akthon.JPEG")
        _, groupid = await create_supergroup(
            "مجموعة أشعارات أكثون ", jepiq, Config.TG_BOT_USERNAME, descript, photobt
        )
    ...
    if PM_LOGGER_GROUP_ID != -100:
        ...
        photobt = await jepiq.upload_file(file="JepIQ/razan/resources/start/Akthon2.JPEG")
        _, groupid = await create_supergroup(
