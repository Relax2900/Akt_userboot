...
def main_menu():
    text = f"**- [سـورس أكثون](https://t.me/VV744)\
        \n المسـاعد\
        \n\
        \n◽ أكثون لـ {mention}**"
    buttons = [
...
async def inline_handler(event):  # sourcery no-metrics
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("jepiq"):
            buttons = [
                (
                    ...
                    Button.url(" Akthon UsᴇʀBoᴛ", "https://t.me/VV744"),
                )
            ]
            ...
            result = builder.article(
                title="رسـالة تـرول من سـورس أكثون 🧸♥",
            ...
            result = builder.article(
                title="همسـة سـرية من سـورس أكثون 🧸♥",
                text=f"فقـط {razan} هـو مـن يستـطيع مشـاهدتهـا !",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            ...
            result = builder.article(
                title="رسـالة مخفيـة من سـورس أكثون 🧸♥",
                text=f"✖✖✖",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
        ...
        text, msg_entities = await event.client._parse_message_text(
            "**[بـوت أكثون 🧸♥](https://t.me/VV744)**\
            \n\
            \n❤ أكثون هـو بـوت بسـيط يدخـل الـى حسـابك لجعـلك تتحـكم به معـ اوامـر معينـة.\
            \n\
            \n**اذا كـنت تـرغب بتـنصيـب السـورس عـلى حسـابك ,\
            \n🐾 تعال الـى [كروب المسـاعدة](https://t.me/AkTSupport)!**",
            "md",
        )
        result = types.InputBotInlineResult(
            id=str(uuid4()),
